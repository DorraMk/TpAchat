 ...
 * .of(new CategorieProduit("cat2", "categorie 2"), new CategorieProduit("cat2",
 * "categorie 3"))
 * .collect(Collectors.toList()));
 * assertEquals(2,
 * categorieProduitServiceImpl.retrieveAllCategorieProduits().size());
 * List<CategorieProduit> listCategorie =
 ...
 * ));
 * verify(categorieProduitRepository,
 * times(1)).deleteById(cat.getIdCategorieProduit());
 ...
 * log.info("==>size:" + listCategorie.size());
 * for (int i = 0; i < listCategorie.size(); i++) {
 * categorieProduitServiceImpl.deleteCategorieProduit(listCategorie.get(i).
 * getIdCategorieProduit());
 * ;
 * log.info("==> categorie " + listCategorie.get(i).getLibelleCategorie() +
 * " deleted successfulyy ");
 * }
 * }
 * }
 */